      ...
      register(user)
        .then((prom) => {
          if (prom.data.status == "VALID") {
            setTimeout(() => {
              success.current.innerHTML = `<p>A verification link has been sent to <b>${user.email}</b></p>`;
          ...
          if (prom.data.status == "ERROR") {
            setTimeout(() => {
              ...
              success.current.innerHTML = `<p>Sorry it looks like something went wrong.. <b>Try again</b></p>`;
              success.current.style.display = "flex";
            });
          }
